...
Package path after scaffold: bestsellerfurniture/store/app
...
- `ui/theme/Theme.kt` — uses BSSFT naming → apply brand colors
- `ui/theme/Type.kt` — typography → customize if needed
- `BSSFTApp.kt` — Application class (already renamed)
...
- `data/database/BSSFTDatabase.kt` — Room DB
- `data/database/converter/Converters.kt` — type converters
- `data/datastore/BSSFTOnboardingPrefs.kt` — onboarding flag
...
- `data/repository/BSSFTOnboardingRepo.kt` — onboarding state
...
- `ui/composable/shared/*` — BSSFTContentWrapper, BSSFTEmptyView
